...
For instance:
[Erlang/OTP 28 - Erts Release Notes - Version 15.0](https://www.erlang.org/doc/apps/erts/notes.html#erts-15.0).

This year's highlights mentioned in this blog post are:

* [Priority Messages](#priority-messages)
* [Improvements of Comprehensions](#comprehension-improvements)
* [Nominal Types](#nominal-types)
* [PCRE2](#pcre2)
* [New `hibernate/0`](#hibernate0)
* [Based Floating Point Literals](#based-floats)
* [Warnings for Use of Old-style Catch](#warn-old-catch)
* [Smarter Error Suggestions](#error-suggestions)
* [Improvements to the Shell](#shell-improvements)
...
Previously, all messages are inserted to the end of the message queue of
...
be inserted before ordinary messages in the message queue. The same process
...
has been implemented. This language feature aims to improve both expressiveness
and safety.
...
Using strict generators are not always better than using relaxed generators,
and vise versa. They each have their use cases, and in other cases using
either is fine. Strict generators are useful when consistency within the
data is more important than fault tolerance. On the other hand, relaxed
generators remain good when you want to skip unmatched data.
...
Zip generator as described in [EEP 73](https://www.erlang.org/eeps/eep-0073)
has been implemented, making it easier to iterate over multiple lists,
binaries, or maps in parallel.
...
has been implemented in Dialyzer. Nominal typing is an alternative type
system. Nominal types can be declared using `-nominal`. The main use case
of nominal types is to prevent accidental misuse of types with the same
structure.
...
There are exceptions to the nominal type-checking rules shown above. For more details, see [Nominals](system/doc/reference_manual/nominals.md) in the reference manual.
...
This upgrade offers better long-term maintainability. On the other hand,
you may need to test your existing regex code before upgrading.


# New `hibernate/0`

Erlang/OTP 28 introuces a new `hibernate/0`. This built-in function puts
the calling process into a wait state where its memory footprint is reduced
as much as possible. When the process receives its next message, it will
wake up. Unlike the existing `erlang:hibernate/3`, it does not discard the
call stack.
